...
export function GoodsReceiptDetailDrawer({ receipt, onClose }: Props) {
  ...
  const handlePrint = () => {
    toast.success(`Đang in phiếu nhập ${receipt.number}...`);
    setTimeout(() => window.print(), 200);
  };

  const handlePrintBarcodes = () => {
    toast.success(`Đang in ${lines.length} tem mã vạch...`);
  };
    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative w-full max-w-md bg-card border-l shadow-xl flex flex-col animate-slide-in-right">
        ...
        <div className="p-4 border-t flex gap-2">
          <button onClick={handlePrintBarcodes} className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 text-sm font-medium border rounded-md hover:bg-muted">
